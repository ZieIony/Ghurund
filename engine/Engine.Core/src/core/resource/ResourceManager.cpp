...
namespace Ghurund::Core {
    ...
    void ResourceManager::saveInternal(Resource& resource, Loader& loader, File& file, const ResourceFormat* format, SaveOption options) const {
        MemoryOutputStream stream;
        try {
            loader.save(*this, stream, resource, format, options);
        } catch (std::exception& exception) {
            Logger::log(LogType::ERR0R, std::format(_T("failed to save file: {}\n"), file.Path).c_str());
            throw exception;
        }
