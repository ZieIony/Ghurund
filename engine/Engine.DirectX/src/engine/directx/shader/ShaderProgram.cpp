...
namespace Ghurund::Engine::DirectX {
	AString ShaderProgram::compile(const AString& code, const FilePath* filePath) {
		...
		AString fileNameA;
		if (filePath) {
			localShaderDir = filePath->Directory.toString().Data;
			fileNameA = convertText<wchar_t, char>(filePath->FileName.Data);
		}
		CompilerInclude include(localShaderDir, L"shaders/");
		AString targetText = makeCompilationTarget();
		ID3DBlob* errorBlob;
		ComPtr<ID3DBlob> shader;
		HRESULT hr = D3DCompile(code.Data, code.Length, fileNameA.Data, nullptr, &include, entryPoint.Data, targetText.Data, compileFlags, 0, &shader, &errorBlob);
		if (FAILED(hr)) {
