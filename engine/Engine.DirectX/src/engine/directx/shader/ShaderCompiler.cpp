...
namespace Ghurund::Engine::DirectX {
	...
	D3D12_INPUT_LAYOUT_DESC ShaderCompiler::getInputLayout(const Buffer& byteCode) {
	...
	DXGI_FORMAT ShaderCompiler::getFormat(BYTE mask, D3D_REGISTER_COMPONENT_TYPE componentType) {
		if (mask == 1) {
	...
	ShaderProgram* ShaderCompiler::compile(const AString& sourceCode, const ShaderType& shaderType, NotNull<CompilerInclude> include, bool debug) {
		...
		if (FAILED(hr)) {
			if (errorBlob == nullptr) {
				...
				throw CompilationException(hr, "Unknown error while compiling shader");
			} else {
	...
	ID3D12PipelineState* ShaderCompiler::makePipelineState(
		...
		for (auto& program : programs) {
			if (program->Type == ShaderType::VERTEX) {
