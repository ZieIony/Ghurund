...
namespace Ghurund {
	Status DrawableComponent::loadInternal(const DirectoryPath& workingDir, MemoryInputStream& stream, LoadOption options) {
		/*Status result;
	...
	Status DrawableComponent::saveInternal(const DirectoryPath& workingDir, MemoryOutputStream& stream, SaveOption options) const {
		/*if (mesh == nullptr)
