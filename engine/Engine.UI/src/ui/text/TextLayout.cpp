...
namespace Ghurund::UI {
    Status TextLayout::buildLayout(IDWriteFactory& dwriteFactory) {
    ...
    void TextLayout::copySinglePropertyRange(IDWriteTextLayout* oldLayout, uint32_t startPosForOld, IDWriteTextLayout* newLayout, uint32_t startPosForNew, uint32_t length, Ghurund::UI::TextFormat* textFormat) {
    ...
    Status TextLayout::insertTextAt(IDWriteFactory& dwriteFactory, uint32_t position, const WString& textToInsert) {
        ...
        Status result = buildLayout(dwriteFactory);
    ...
    Status TextLayout::removeTextAt(IDWriteFactory& dwriteFactory, uint32_t position, uint32_t lengthToRemove) {
        if (layout)
            layout->AddRef();
        IDWriteTextLayout* oldLayout = layout;
        uint32_t oldTextLength = text.Length;
    ...
    Ghurund::UI::Color TextLayout::getColor(uint32_t pos) {
        ...
        if (effect) {
            Ghurund::UI::Color color = ((DrawingEffect*)effect)->Color;
            effect->Release();
            return color;
        }
        return Color;
    ...
    TextFormat* TextLayout::getFormat(uint32_t position) {
        ...
        return ghnew Ghurund::UI::TextFormat(L"", fontFamilyName.begin(), size, fontWeight, fontStyle == DWRITE_FONT_STYLE_ITALIC, localeName.begin());
    }
