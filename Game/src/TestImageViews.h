    ...
    TestImageViews(Ghurund::UI::Theme& theme) {
        ...
        Name = "image views tab";
        {
            ...
            for (unsigned int i = 0; i < 4; i++) {
                ...
                ImageViewPtr imageView = ghnew ImageView();
                imageView->Image = image;
                imageView->ScaleMode = (enum ImageScaleMode)i;
                stack->Children = { imageView, border };
                TextBlockPtr textView = ghnew TextBlock(theme.textViewSecondaryStyle);
                textView->Text = texts[i];

                column->Children.addAll({ textView, stack, makeSpace(16.0f) });
            }
