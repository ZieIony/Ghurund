...
namespace Ghurund {
    ...
    public:
        static void registerClass(asIScriptEngine& engine, Timer** timer) {
            ...
            engine.RegisterObjectType(Type::TIMER.Name, sizeof(Timer), asOBJ_REF|asOBJ_NOCOUNT);
            auto r = engine.RegisterObjectMethod(Type::TIMER.Name, "float getTime()", asMETHOD(Timer, getTime), asCALL_THISCALL); assert(r >= 0);
