...
namespace Ghurund::UI {
    ...
    private:
        ...
        public:
            ...
            AdapterWithPool(ItemAdapter* adapter):adapter(adapter) {}
        ...
        AdapterWithPool<T>* findAdapterForChild(const T& child) {
            for (AdapterWithPool<T>* a : adapters) {
                if (a->adapter->typeInfo == typeid(child))
                    return a;
            }
            return nullptr;
        }

        Control* makeControl(size_t index) {
            return findAdapterForChild(items[index])->getControl();
        }

        void recycle(Control* control, size_t index) {
            findAdapterForChild(items[index])->recycle(control);
        }
    ...
    public:
        ...
        void onLayout(float x, float y, float width, float height) {
            layoutChildren();
        }
