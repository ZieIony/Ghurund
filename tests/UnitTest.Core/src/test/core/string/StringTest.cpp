...
public:
	...
	TEST_METHOD(String_replaceAll) {
		MemoryGuard guard;
		{
			...
			replacedText.replaceAll(L"test", L"plum");
			...
			replacedText2.replaceAll(L"test", L"lemon");
	...
	TEST_METHOD(String_iterator) {
		...
		{
			String str = _T("test");
			List<tchar> a = { _T('t'), _T('e'), _T('s'), _T('t') };
			List<tchar> b;
			for (auto c : str)
				b.add(c);
			Assert::AreEqual(a, b);
	...
	TEST_METHOD(String_reverseIterator) {
		MemoryGuard guard;
		{
			String str = _T("test");
			List<tchar> a = { _T('t'), _T('s'), _T('e'), _T('t') };
			List<tchar> b;
...
namespace Microsoft::VisualStudio::CppUnitTestFramework {
	template<> static std::wstring ToString<Ghurund::Core::List<tchar>>(const Ghurund::Core::List<tchar>& t) {
		std::wstring str;
		for (auto c : t) {
			auto s = Ghurund::Core::String();
			s.add(c);
			str.append(convertText<tchar, wchar_t>(s).Data);
		}
		return str;
	}
}