...
namespace UnitTest::Utils {
    ...
    T* makeComponent(World2D& world) {
        auto owner = runCoroutineBlocking(world.spawnEntity<Entity2D>());  // owner gets released because the coroutine is already destroyed, but it's fine, because owner has more references elsewhere
