...
namespace Material {
    ...
    public:
        Theme(Ghurund::ResourceManager& resourceManager, Ghurund::ResourceContext& context) {
            auto buttonFont = Ghurund::makeShared<Ghurund::UI::TextStyle>("fonts/lato_medium.ttf", "Lato Medium", 10.0f, FW_MEDIUM, false);
            buttonFont->init(context.Graphics2D);
            TextStyles.set(Theme::TEXTSTYLE_BUTTON, buttonFont);

            auto listHeaderFont = Ghurund::makeShared<Ghurund::UI::TextStyle>("fonts/lato_light.ttf", "Lato Light", 14.0f, FW_LIGHT, false);
            listHeaderFont->init(context.Graphics2D);
            TextStyles.set(Theme::TEXTSTYLE_LIST_HEADER, listHeaderFont);

            auto textPrimaryFont = Ghurund::makeShared<Ghurund::UI::TextStyle>("fonts/lato_regular.ttf", "Lato", 11.0f, FW_REGULAR, false);
            textPrimaryFont->init(context.Graphics2D);
            TextStyles.set(Theme::TEXTSTYLE_TEXT_PRIMARY, textPrimaryFont);

            auto textSecondaryFont = Ghurund::makeShared<Ghurund::UI::TextStyle>("fonts/lato_regular.ttf", "Lato", 10.0f, FW_REGULAR, false);
